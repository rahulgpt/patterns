                ...
                <div className='auth-wrapper'>
                    ...
                    <form className='auth-form' onSubmit={this.handleSubmit}>
                        <label id='auth-username'>
                            Username or Email
                        <input ref={this.signInUsername} type='text' name='authUsername' className='auth-input' onChange={this.handleInputChange} />
                        </label>
                        <label id='auth-password'>
                            Password
                        <input type='password' name='authPassword' onChange={this.handleInputChange} />
                            <p>{this.props.error ? this.props.error : ' '}</p>
                        </label>
                        <div className='auth-btn-wrapper'>
                            {this.props.loading ?
                                <button id='loader-btn' type='submit'>{loader2}</button> :
                                <button className='auth-btn' type='submit' onClick={this.handleSignInSubmit}>SIGN IN</button>
                            }
                    ...
                    <p id='reg-benefit'>Sign up for an account to take advantage of
                    order tracking and history as well as pre-filled forms during
                    checkout on subsequent orders.</p>
                    <Link to='/register'><button className='auth-btn reg-btn'>REGISTER</button></Link>
                </div>
            </div>
        );
    }
