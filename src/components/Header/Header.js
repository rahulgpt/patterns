        ...
        const signInMenu = (
            <div className='signin-container'>
                ...
                {localStorage.getItem('_shpuid') ?
                    (this.props.loading ? loader2 : <Link to='/shop' onClick={this.props.authSignOut} className='margin'>Sign Out</Link>)
                    : <Link className='margin' to='/signin' >Sign In</Link>}
                <div id='user-menu'>
                    <img className
                        ='avatar' src={UserIcon} onClick={this.props.toggleUserMenu} />
        ...
        return (
            <div className='header-container'>
                <div>
                    ...
                    <p className='page-nav'>Home {" > "} {this.props.title}</p>
                </div>

                <div className='signin-filter-container'>
                    {signInMenu}
                    <a href='#' className="filter">Filter <img className='down-arrow' src={DownArrowIcon} /></a>
                </div>
            </div>
        );
    }
